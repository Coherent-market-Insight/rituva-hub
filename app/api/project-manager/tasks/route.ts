...
export async function GET(request: NextRequest) {
  try {
    ...
    // Project Manager sees tasks pushed to AM and tracks tasks they've acted on
    // Includes sent_for_review_by_pm so tasks stay visible after PM pushes back to admin
    const tasks = await prisma.task.findMany({
      where: {
        status: {
          in: ['push_to_account_manager', 'push_to_client', 'sent_for_review', 'sent_for_review_by_pm', 'client_completed'],
        },
      },
      include: {
        creator: {
